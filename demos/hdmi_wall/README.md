...
屏幕字段逐项解释见 [32 路视频墙指标说明](docs/wall-indicators.md)，包括解码 / 检测 FPS、TPU、LAG、AGE、src age 和异常状态。
...
普通视频墙现在直接显示解码指标，保留全部通道图像：顶部为总解码 `DEC`、输入目标 `TARGET`、总推理 `INF` FPS；每格为该路的 `DEC / INF`、解码落后 `LAG`、距最后一次解码的 `AGE` 及解码状态。`LAG` 以本地源播放时钟为基准，RTSP 未知时显示 `--`；`SLOW / STALL` 表示解码降速 / 停顿，`STALE` 仍指检测画面过旧。解码计数位于抽帧过滤之前，不需要开启对照布局，也不会为此增加解码原图回读。

多设备播放器的设备按钮显示近期实测 TPU 利用率。`showcase.sh` 默认采集，普通 `multi_run.sh` 需加 `--telemetry-interval 5`；未启用、读取失败或样本过期显示 `TPU --`。两个视频墙 FPS 使用最近完整的 2 秒分桶窗口，更新画面不会继续沿用停止前的非零 FPS。
