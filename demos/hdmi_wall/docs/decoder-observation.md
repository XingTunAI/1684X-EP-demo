...
比较“推理开 / 关”和比较“结果回传开 / 关”是两个实验。新 [READBACK 开关](readback-toggle.md)在 OFF 时仍执行真实帧推理；`observe --inference off` 则不执行模型，不能混用。跨实验对照见 [数据总览](current-data.md)。
...
解码基线与推理组应使用相同输入、路数及观察参数，并记录正式窗口内 TPU 利用率，确认推理组是否实际达到所要求的负载。下文这次 observe 配置的双卡 32 路实测仅 Gen3 ×2 接近 TPU 满载；Gen2 ×1 不能以“命令要求满载”替代其实际约 79% 的结果。
