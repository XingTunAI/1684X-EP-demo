...
整墙另增 `--wall-fps 0`，取消合成线程主动限频；默认10，正数允许至120。状态 `preview_fps_cap=0` 表示整墙不限频，配置 `per_stream_preview_fps=-1` 表示单路不限频。本次实验同时设置这两个参数，独立播放器输入时基改为60FPS。整墙帧可能重复，实际有效预览帧率应看小图回传字节增量与各路更新，不能用整墙刷新次数代替。

底层 `hdmi_wall.pcie --preview-fps -1` 表示不主动限制单路检测预览频率：同步模式下每次检测完成均生成并回传小图；异步模式仍使用最新单槽，不能保证每个检测结果都有预览。`0` 仍表示关闭预览，正数大于0且不超过10表示限频。单独设置此选项不改变整墙及播放器节奏；需同时配置 `--wall-fps` 和播放器，不保证实际达到源帧率。现有run/multi_run脚本仍限制预览参数为非负值，本次使用独立build-preview-unlimited二进制直接启动。
