...
底层 `hdmi_wall.pcie --preview-fps -1` 表示不主动限制单路检测预览频率：同步模式下每次检测完成均生成并回传小图；异步模式仍使用最新单槽，不能保证每个检测结果都有预览。`0` 仍表示关闭预览，正数0到10仍表示限频。此选项不修改整墙合成和播放器10 FPS刷新节奏，也不保证实际预览帧率。脚本入口可能仍限制非负值，本次使用独立build-preview-unlimited二进制直接启动。
