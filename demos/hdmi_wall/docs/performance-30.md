...
本页是 2026-09-09 单卡历史基准；benchmark 入口用于复现。当前双卡各 32 路推荐见 [数据总览](current-data.md)。
...
先完成[环境和资源准备](../../../docs/setup.md)，确认 `bm-smi --noloop` 中 device 1 可用，并重新编译。下面命令在板端执行，适用于已确认 Xorg 使用 `:0`、认证文件为 `/var/run/lightdm/root/:0` 的 LightDM 系统；SSH 普通用户使用完整的 `sudo env`，ADB root 可去掉 `sudo`。其他桌面会话按[连接与显示说明](display.md)选择实际地址和认证文件。
