...
单独隔离检测缩略图开销，可在启动时设 `--preview-fps 0`：保留模型输出回传和 CPU 检测后处理，停止检测缩略图生成、回传和画框。页面不再有新的检测预览。这与 READBACK OFF 不同；`observe-decode` 对比通道的原图观察仍由 `--observe-preview-fps` 独立控制。默认显示配置不变。
