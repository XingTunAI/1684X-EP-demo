...
多设备启动器为各卡传入同一个 `--readback-control` JSON。检测线程在每次处理开始时取得本帧模式，OFF 仍对真实解码输入预处理并推理，随后跳过结果消费；已经开始的 ON 处理允许完成，以 `readback_inflight` 为切换完成依据。预览路径受同一开关控制。

`completed` 包括 ON / OFF 两类模型完成，正式区间分别记录 `results_readback_measured` 与 `model_only_measured`；OFF 的逐帧 `detections` 为 null。详见 [统计字段](../../../docs/metrics.md#hdmi-回传开关与区间统计)。
...
先构建并用单路短视频验证同帧检测框与反复 EOF 循环，再以相同输入/模型对比 `all` 与 `latest`。2026-09-09 公路素材阶段的已测起点为 24 路 / 5 FPS 或 16 路 / 8 FPS，当时 32 路未满足该轮实时要求；后续官方 1080p24 的双卡 32 路结果见 [当前数据](current-data.md)，不是同条件复测。关注连续窗口内的解码 FPS、检测 FPS、源年龄 P95、主动丢帧数和内存使用；最新槽高水位应不超过 1，正常结束未处理帧数应为 0。以更低检测限频制造慢消费者，验证不会回放历史积压帧。
