...
首次大文件准备包含复制、faststart 整理和完整文件校验，会产生大量磁盘 I/O，应先准备完成再开始性能压测。复用 verified cache 时仍完整计算源文件和成品的 SHA256，需要等待；不要因暂时没有 HDMI 画面而重复启动。素材准备与校验时间不计入正式四小时。素材校验成功不代表四小时运行已经完成或通过验收。
