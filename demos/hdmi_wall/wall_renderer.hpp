...
public:
    WallRenderer(size_t count, const std::string& output_dir, int device,
        ...
        : count_(count), output_dir_(output_dir), device_(device),
          model_name_(model_name), frames_(count), stopped_(false),
    ...
    void submit(size_t id, const cv::Mat& host_bgr, uint64_t frame_count,
                size_t detections, double process_fps) {
...
private:
    ...
    cv::Mat render(const std::vector<Frame>& frames, Time now) const {
        ...
        title << count_ << "CH  /  " << model_name_ << "  /  Device " << device_;
        label(canvas, title.str(), 20, 32, .8, white, 2);
        label(canvas, "Independent decode + inference | Live bounding boxes | HDMI preview 10 FPS", 20, 58, .48, gray);
        for (size_t id = 0; id < 36; ++id) {
            ...
            cv::rectangle(canvas, cv::Rect(x + 2, y + 145, 316, 21), cv::Scalar(16, 15, 12), -1);
            ...
            label(canvas, bottom.str(), x + 9, y + 160, .41, gray);
