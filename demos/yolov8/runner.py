...
report_spec = importlib.util.spec_from_file_location('_sophon_yolov8_report', Path(__file__).with_name('report.py'))
report = importlib.util.module_from_spec(report_spec)
report_spec.loader.exec_module(report)
write_report = report.write_report

spec = importlib.util.spec_from_file_location('_sophon_decode_bench', ROOT / 'demos/decode/stress_decode.py')
...
def main(argv=None):
    ...
    if a.output == ROOT / 'data/results/decode':
        a.output = ROOT / ('data/results/analysis' if a.mode == 'analysis' else 'data/results/pipeline')
