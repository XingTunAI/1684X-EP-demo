...
spec = importlib.util.spec_from_file_location('pipeline_run', Path(__file__).resolve().parents[1] / 'runner.py')
pipeline = importlib.util.module_from_spec(spec)
spec.loader.exec_module(pipeline)


class VerificationTests(unittest.TestCase):
    def test_output_directory_reaches_decode_parser(self):
        with tempfile.TemporaryDirectory() as tmp:
            output = Path(tmp) / 'card output'
            for option in (['--output', str(output)], ['--output=' + str(output)]):
                with self.subTest(option=option), patch('sys.stdout', new_callable=io.StringIO) as captured:
                    code = pipeline.main(['--bmodel', 'example.bmodel', '--mode', 'analysis',
                        '--device', '3', '--steps', '2', '--input', 'example.mp4',
                        '--warmup', '0', '--duration', '300', '--window', '10', '--dry-run'] + option)
                self.assertEqual(code, 0)
                command = json.loads(captured.getvalue())['example']
                self.assertIn('--output=' + str((output / 'stream_00').resolve()), command)
                self.assertIn('--output_buffer=baseline', command)
            ...
            with (stream / 'detections.jsonl').open('w') as out:
                for frame in frames:
                    out.write(json.dumps({'frame': frame, 'completed_monotonic_s': 11 + frame,
                        ...
                        'analysis_ms': 10, 'inference_submit_ms': 0.1, 'inference_sync_ms': 2.8,
                        'input_release_ms': 0.02, 'output_transfer_ms': 3, 'cpu_postprocess_ms': 2,
    ...
    def test_analysis_without_video(self):
        ...
        self.assertEqual(result['mean_stage_ms']['inference_submit_ms'], 0.1)
        self.assertEqual(result['mean_stage_ms']['inference_sync_ms'], 2.8)
        self.assertEqual(result['mean_stage_ms']['input_release_ms'], 0.02)
    ...
    def test_report_preserves_slow_window_and_unmeasured_encoder(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(row['inference_submit_mean_ms'], '')
            self.assertEqual(row['inference_sync_mean_ms'], '')
            self.assertEqual(row['input_release_mean_ms'], '')
