...
2026-09-07 18:23 复查：系统枚举到一张算能设备，`0001:11:00.0` 对应软件 device 0，实际链路为 `Speed 8GT/s, Width x2`。主控根端口 `0001:10:00.0` 支持 ×2；模组端可支持更宽链路，所以模组端显示 `x2 (downgraded)` 不等于当前没有达到主控的 ×2。此前双卡配置中，该 BDF 对应 device 1 / PCIe 3.0 ×1。

以下双卡拓扑和安装记录为历史参考。设备编号与实际链路需每次重新核对，不能只按插槽名称推断。
