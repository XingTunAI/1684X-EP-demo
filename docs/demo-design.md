...
本阶段实现的是纯解码压测与解码＋推理分析，默认只测性能。32 路分析输出检测 JSON 和报告，不画框、不编码、不实时显示、不抽帧；总平均 114.10 FPS。完整编码模式和已有多卡样例另有历史验证。当前交付入口见[演示总览](demo-summary-20260907.md)。下文保留整体设计参考，UI、RTSP 推送等设想不能作为本轮已交付功能。
