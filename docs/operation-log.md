...
## 单卡 Demo 当前记录（2026-09-07）

纯解码阶梯和 30 路 15 分钟测试已完成；长测总平均 749.96 FPS，有一个窗口波动，历史 fps_fail 保留。32 路分析经过设备图像路径优化，总平均 114.10 FPS、每路平均 3.57 FPS。独立模型计算 TPU 采样峰值达到 100%。设备端解码 18 项、流水线 10 项测试已通过。

当前默认只测性能，未追加抽帧、实时显示或客户素材测试。视频、模型和原始结果已另行同步，本仓库提交代码和结论文档。复测、统计与当前状态分别见[纯解码操作](single-card-decode.md)、[分析操作](single-card-analysis.md)、[报告指南](benchmark-report-guide.md)、[演示总览](demo-summary-20260907.md)。以下记录保留先前环境准备和功能验证历史。
