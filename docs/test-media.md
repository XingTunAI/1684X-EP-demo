...
纯解码一键入口默认使用 596.44 秒短视频，适合每档 60 秒预热＋120 秒采集；30 路 15 分钟正式采集需显式选择 20 分钟版本。分析一键入口默认已使用 20 分钟版本。短视频循环附近曾出现送包错误，换长素材仅避免本轮触及 EOF，并未修复循环路径。

32 路重复读取同一动画，用于受控演示，不证明摄像头接入或客户算法精度。当前阶段不追加业务素材测试。复测命令见[纯解码](single-card-decode.md)和[分析操作](single-card-analysis.md)。

## 原始视频

原始 `big_buck_bunny_1080p_h264.mov` 保留在本机素材准备目录，未作为工程交付文件。工程内保存下面两份 25 FPS 成品及校验记录。重新生成时，先把原片放到命令指定的输入位置或调整输入参数。
