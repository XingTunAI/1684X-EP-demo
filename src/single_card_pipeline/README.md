...
全程粗分阶段平均耗时：解码/取帧 57.87 ms，分析（含输入转换、预处理、推理、后处理）65.44 ms，画框 0.12 ms，编码提交 92.58 ms。不能把分析阶段耗时直接当成 TPU 纯推理耗时，也不能把编码提交耗时全部归因于编码硬件。后续先排查主机与设备间图像搬运、OpenCV 编解码路径及模型实现，再重新测单路，当前不建议直接冲 32 路。
