...
def main():
    ...
    p.add_argument('--expected-link-speed',choices=['5.0','8.0'],required=True)
    ...
    cases=[('none','sync',0),('sync10','sync',10),('async3','async',3)]
    if a.reverse: cases.reverse()
    ...
    try:
        for rep,(case,preview_stage,rate) in plan:
            ...
            if case != 'original':
                cmd[0]=str(ROOT/'demos/hdmi_wall/build-async/hdmi_wall.pcie')
