#!/usr/bin/env python3
"""Isolate preview VPP, readback and display using the same diagnostic binary."""
import argparse
...
def main():
    ...
    import hashlib
    binary=ROOT/'demos/hdmi_wall/build-ablation/hdmi_wall.pcie'
    evidence['diagnostic_binary_sha256']=hashlib.sha256(binary.read_bytes()).hexdigest()
    save(a.output/'preflight.json',evidence)
    save(a.output/'state.json',{'status':'running'})
    cases=[('original','display',10),('none','display',0),
           ('vpp','vpp',10),('readback','readback',10),('display','display',10)]
    ...
    rows=[]
    try:
        for rep,(case,preview_stage,rate) in plan:
            gate,budget,readback,buffer="on",64,True,"reuse"
            name=f'r{rep}_{case}'
            cmd=command(cfg,'wall_preview')
            if case != 'original':
                cmd[0]=str(ROOT/'demos/hdmi_wall/build-ablation/hdmi_wall.pcie')
                cmd+=['--preview-stage',preview_stage]
